...
void SDUStringReplaceN(char *Text, size_t Length, char *OldString, char *NewString)
{
    ...
    while (!(finished))
    {
        ...
        {
            // -1 to include terminating NULL char
            strncat(Text, tmpHead, min(((currFound - tmpHead) / sizeof(char)), ((Length - strlen(Text)) - 1)));
            // -1 to include terminating NULL char
            strncat(Text, NewString, min(strlen(NewString), ((Length - strlen(Text)) - 1)));
...
void SDUStringReplaceNW(WCHAR *Text, size_t Length, WCHAR *OldString, WCHAR *NewString)
{
    ...
    while (!(finished))
    {
        ...
        {
            // -1 to include terminating NULL WCHAR
            wcsncat(Text, tmpHead, min((unsigned int)((currFound - tmpHead)), ((Length - wcslen(Text)) - 1)));
            // -1 to include terminating NULL WCHAR
            wcsncat(Text, NewString, min(wcslen(NewString), ((Length - wcslen(Text)) - 1)));
            tmpHead += (currFound - tmpHead);
